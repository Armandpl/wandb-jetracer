...
if __name__=="__main__":
    parser = argparse.ArgumentParser(description='tool to label images for regression')
    ...
    with wandb.init(project=args.project, config=args, entity=args.entity, job_type="labelling") as run:

        config = run.config

        #TODO check if latest tag needed
        artifact_loc = '{entity}/{proj}/{ds}:latest'.format(entity=config.entity, proj=config.project, ds=config.dataset)
        ...
        try:
            for fname in os.listdir(artifact_dir):
                # not fname.contains("_") is our way of knowing if the image is already labeled
                if fname.endswith(".jpg") and not "_" in fname:
                    # reading the image
